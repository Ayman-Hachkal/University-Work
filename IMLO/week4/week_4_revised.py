...
def E_n(h, data, labels, L, theta, theta_0):
    ...
    error = 0
    for i in range(n):
        error += L(h(data[:, [i]], theta, theta_0), labels[0, i])
    return error/n
...
def random_linear_classifier(data, labels, params={}, hook=None):
    ...
    for j in range(k):
        ...
        error = E_n(linear_classify, data, labels, Loss, theta, theta_0)
...
def perceptron(data, labels, params={}, hook=None):
    ...
    for t in range(T):
        if hook: hook((theta, theta_0))
        for i in range(n):
            if linear_classify(data[:, i], theta, theta_0)*labels[0, i] <= 0:
                theta += labels[0, i] * data[:, [i]]
                theta_0 += labels[0, i]
...
def plot_separator(plot_axes, theta, theta_0):
    ...
    # plot_axes.arrow((xmin + xmax) / 2, (p1_y.flatten()[0] + p2_y.flatten()[0]) / 2, float(theta[0]), float(theta[1]))                     # Old line of code that throws an error in NumPy 2.4.2
    plot_axes.arrow((xmin + xmax) / 2, (p1_y.flatten()[0] + p2_y.flatten()[0]) / 2, theta.astype(float)[0, 0], theta.astype(float)[1, 0])   # New line of code that works in NumPy 2.4.2
...
if __name__ == '__main__':
    ...
    # Disables matplotlib interactive mode and keeps plot window open until it is closed.
    plt.ioff()
    plt.show()
    print("Finished.")
